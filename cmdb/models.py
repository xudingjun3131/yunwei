...
class Server_Group(models.Model):
    # 定义主机组名称字段
    name = models.CharField('主机组', max_length=255, unique=True)
    # 关联的项目字段，这是关联一个外键
    project = models.ForeignKey("Project", verbose_name='项目名称')
    # 备注字段
    memo = models.CharField('备注', max_length=255, blank=True)
    # unicode返回值
...
class IDC(models.Model):
    # 定义IDC的名称字段
    name = models.CharField('IDC名称', max_length=255, unique=True)
    memo = models.CharField('备注', max_length=255, blank=True)
...
class Server_Role(models.Model):
    name = models.CharField('角色', max_length=255)
...
class Server_Device(models.Model):
    ...
    name = models.CharField('主机名称', max_length=100, unique=True)
    # 定义SN编号字段， blank=True可以为空
    sn = models.CharField('SN号', max_length=200, blank=True)
    # 公网IP字段，可以为空
    public_ip = models.CharField('外网IP', max_length=200, blank=True)
    # 私网IP，可以为空
    private_ip = models.CharField('内网IP', max_length=200, blank=True)
    # 定义mac地址字段
    mac = models.CharField('MAC地址', max_length=200, blank=True)
    # 定义操作系统字段
    os = models.CharField('操作系统', max_length=200, blank=True)
    # 定义磁盘信息字段
    disk = models.CharField('磁盘', max_length=200, blank=True)
    # 定义内存信息字段
    mem = models.CharField('内存', max_length=200, blank=True)
    # 定义CPU信息字段
