        ...
        try:
            ...
            if type(f) in [ForeignKey, OneToOneField]:
                ...
                if not obj:
                    ...
                    f_val = list(_f_data.keys())
                    f_text = ", ".join(list(_f_data.values()))
            ...
            logger.exception("get_data error: %s" % e)
...
def get_manytomanys_edit_context(model_name, model_obj=None):
    ...
    for f in model_name._meta.many_to_many:
        try:
            ...
            logger.exception("get manytomany error: %s" % e)
            raise Exception(e)
...
def get_page_choices(page_n, page_max):
    ...
    if page_max > r:
        return [0] + list(range(l, r)) + [page_max - 1]
    return [0] + list(range(l, r))